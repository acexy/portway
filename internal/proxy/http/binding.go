...
// RequestResult reports the outcome generated by the proxy runtime itself.
// Upstream application status codes are not classified as proxy failures.
...
type requestResultContextKey struct{}
...
func (writer *statusResponseWriter) Write(content []byte) (int, error) {
	if writer.statusCode == 0 {
		writer.statusCode = stdhttp.StatusOK
	}
	return writer.ResponseWriter.Write(content)
}

func (writer *statusResponseWriter) Unwrap() stdhttp.ResponseWriter {
...
func NewBinding(
	...
	binding.proxy = &httputil.ReverseProxy{
		...
		ErrorHandler: func(writer stdhttp.ResponseWriter, request *stdhttp.Request, err error) {
			...
			if result, ok := request.Context().Value(requestResultContextKey{}).(*RequestResult); ok {
				result.ErrorCode = httpProxyErrorCode(status)
				result.Err = err
			}
...
func (binding *Binding) ServeHTTP(writer stdhttp.ResponseWriter, request *stdhttp.Request) {
	binding.ServeHTTPResult(writer, request)
}

// ServeHTTPResult forwards one request and reports failures generated by Portway.
func (binding *Binding) ServeHTTPResult(
	...
	if binding.maxRequestBodyBytes > 0 && request.ContentLength > binding.maxRequestBodyBytes {
		...
		stdhttp.Error(statusWriter, "Request Entity Too Large", stdhttp.StatusRequestEntityTooLarge)
		result.StatusCode = statusWriter.statusCode
		return result
	}
	...
	if request.Body != nil {
		if binding.maxRequestBodyBytes > 0 {
			request.Body = stdhttp.MaxBytesReader(statusWriter, request.Body, binding.maxRequestBodyBytes)
		}
		if binding.requestBodyTimeout > 0 {
			request.Body = newTimedRequestBody(request.Body, binding.requestBodyTimeout, cancelRequest)
		}
	}
	requestContext = context.WithValue(requestContext, requestResultContextKey{}, &result)
	binding.proxy.ServeHTTP(statusWriter, request.WithContext(requestContext))
	result.StatusCode = statusWriter.statusCode
	if result.StatusCode == 0 {
		result.StatusCode = stdhttp.StatusOK
	}
	return result
}
...
func httpProxyErrorCode(statusCode int) string {
	...
	case stdhttp.StatusGatewayTimeout:
