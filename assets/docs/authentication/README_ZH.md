...
完全由服务端集中配置的节点。客户端只需要配置 Token。服务端根据 Token 唯一
选择认证记录、权威身份和配置控制模式。
...
文件名必须严格等于 `<client_id>.yaml`。客户端只配置该记录的 Token 和期望代理：
...
认证后，服务端返回 `customer-a` 作为权威 ClientID，客户端本地声明的 ClientID
不能覆盖它。服务端按照代理类型、TCP/UDP 端口、HTTP 域名、代理数量和活动
Link 限制校验完整代理集合。任意一项越权都会拒绝整批更新，服务端不会静默发布
部分代理。

`proxy_types` 中列出的每种类型都必须配置非空的对应规则：TCP 和 UDP 至少包含
一个 `remote_port_ranges` 区间，HTTP 至少包含一个域名。未列入
`proxy_types` 的类型必须省略对应规则或保持为空。配置多个区间可以分配互不连续
...
Managed 客户端只配置 Token，不能在本地定义 `proxies`：
...
客户端不需要知道服务端启用了哪些模式。Token 选择唯一认证记录，认证成功后，
服务端通过受保护通道返回关联模式和权威 ClientID。
