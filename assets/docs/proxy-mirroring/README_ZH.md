...
新的 TCP 访问者在任一成员链路就绪后即开始转发，不等待所有成员，也不专门等待
Primary。因此，即使客户端已经注册，较晚完成建链的成员仍可能错过流的开头。
如果所有首次建链尝试均失败，则持续读取并丢弃访问者输入，各成员独立重试。

本地服务不可用不会导致客户端退出登录。TCP 成员（包括 Primary）会对失败链路
自动退避重试，本地服务启动或重启完成后继续转发后续字节；不可用期间丢失的数据
及失败链路中排队的数据不重放。Primary 恢复期间保留访问者的响应方向。访问者输入
EOF 才停止重试，并半关闭当前成员链路，最多等待五秒排空及 Primary 响应。UDP
在 Association 失败后通过后续数据报重新建链；未报告错误的 socket 继续正常发送。
两种协议均无需重启 Portway 客户端来恢复本地服务转发。
...
同端口成员或 Primary 更新保留现有 TCP 访问者连接，无关组保持运行。回复资格
原子切换；切换前已获准的单次写入可在原五秒期限内结束，随后新 Primary 才写入。
